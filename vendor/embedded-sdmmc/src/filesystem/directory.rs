    ...
    T: crate::TimeSource,
{
    ...
    pub fn iterate_dir_raw<F>(&mut self, func: F) -> Result<(), Error<D::Error>>
    where F: FnMut(&[u8], Option<&crate::DirEntry>) -> bool,
    {
        self.volume_mgr.iterate_dir_raw(self.raw_directory, func)
    }

    /// Call a callback for each short directory entry.
