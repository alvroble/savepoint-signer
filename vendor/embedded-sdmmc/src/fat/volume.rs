...
impl FatVolume {
    ...
    {
        let mut func = func;
        self.iterate_dir_raw(block_device, dir, |_, entry| {
            if let Some(entry) = entry { func(entry); }
            true
        })
    }

    pub(crate) fn iterate_dir_raw<D, F>(
        &self, block_device: &D, dir: &DirectoryInfo, func: F,
    ) -> Result<(), Error<D::Error>>
    where F: FnMut(&[u8], Option<&DirEntry>) -> bool, D: BlockDevice,
    {
        match &self.fat_specific_info {
            FatSpecificInfo::Fat16(fat16_info) => {
    ...
    where
        F: FnMut(&[u8], Option<&DirEntry>) -> bool,
        D: BlockDevice,
    {
        // Root directories on FAT16 have a fixed size, because they use
        // a specially reserved space on disk (see
            ...
            for block_idx in first_dir_block_num.range(dir_size) {
                ...
                for entry in 0..Block::LEN / OnDiskDirEntry::LEN {
                    ...
                    if dir_entry.is_end() {
                        ...
                        if !func(&block[start as usize..end], Some(&entry)) { return Ok(()); }
                    } else if !func(&block[start..end], None) { return Ok(()); }
                }
            }
            if cluster != ClusterId::ROOT_DIR {
            ...
            for block in block_idx.range(BlockCount(u32::from(self.blocks_per_cluster))) {
                ...
                for entry in 0..Block::LEN / OnDiskDirEntry::LEN {
                    ...
                    if dir_entry.is_end() {
                        ...
                        if !func(&blocks[0][start as usize..end], Some(&entry)) { return Ok(()); }
                    } else if !func(&blocks[0][start..end], None) { return Ok(()); }
                }
            }
            current_cluster = match self.next_cluster(block_device, cluster, &mut block_cache) {
                Ok(n) => Some(n),
