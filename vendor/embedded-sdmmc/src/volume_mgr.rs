    ...
    <D as BlockDevice>::Error: core::fmt::Debug,
{
    ...
    /// Visit raw directory slots, including LFN/deleted entries.
    /// Return false to stop scanning. No data is written.
    pub fn iterate_dir_raw<F>(&mut self, directory: RawDirectory, func: F)
        -> Result<(), Error<D::Error>>
    where F: FnMut(&[u8], Option<&DirEntry>) -> bool,
    {
        let directory_idx = self.get_dir_by_id(directory)?;
        let volume_idx = self.get_volume_by_id(self.open_dirs[directory_idx].volume_id)?;
        match &self.open_volumes[volume_idx].volume_type {
            VolumeType::Fat(fat) =>
                fat.iterate_dir_raw(&self.block_device, &self.open_dirs[directory_idx], func),
        }
    }
