...
async fn main(spawner: Spawner) {
    ...
    let mut builder = {
        ...
        static CONTROL_BUF: StaticCell<[u8; CONTROL_SIZE]> = StaticCell::new();
        ...
        let builder = embassy_usb::Builder::new(
            ...
            CONTROL_BUF.init([0; CONTROL_SIZE]),
        );
        builder
    };
            ...
            move || {
                ...
                let gb_save_ram = unsafe {
                    core::slice::from_raw_parts_mut(
                        ptr::addr_of_mut!(_s_gb_save_ram),
    ...
    // IRQs on this core stay off for the MBC tight loop. Embassy's
    // time-driver alarm also lives here, so core 1 must not await
    // Timer/Ticker — those wakers never fire. Instant::now() still
    // works; it reads the free-running hardware timer.
...
async fn core1_task(
    ...
    // Automatic save worker. Core 0's MBC handler publishes SRAM
    // enable→disable edges; this task owns the SD volume after boot and
    // writes <rom>.TMP then <rom>.SAV in the legacy format the
    // bootloader already restores *before* releasing reset.
    //
    // SRAM enable/disable is not "the user saved": games also map SRAM
    // to read. A restarting quiet period coalesces a save session, and
    // an SRAM CRC skip avoids rewriting the card for identical images
    // (title-screen CONTINUE checks).
    //
    // Do not await embassy Timer/Ticker here. Core 0 has IRQs disabled
    // for the MBC loop, and the time-driver alarm is bound to that core,
    // so those waits never complete. Instant::now() reads the hardware
    // timer directly.
    use cartridge_core::save_coordinator::{
        self, crc32, AutoSaveEngine, PersistDecision, QUIET_PERIOD_MS,
    };
    ...
    const LED_WORKER: RGB8 = RGB8::new(0, 80, 0);
    const LED_WRITE: RGB8 = RGB8::new(0, 0, 80);
    const LED_OK: RGB8 = RGB8::new(0, 80, 0);
    const LED_ERR: RGB8 = RGB8::new(80, 0, 0);
    ...
    #[cfg(feature = "crystal-seed")]
    let mut seed_worker = crystal_seed::Worker::new();
    let mut engine = AutoSaveEngine::new(QUIET_PERIOD_MS);
    engine.on_restored(crc32(saveram_memory));
    info!(
        "save: worker ready for {} ({} KiB SRAM, rtc={})",
        rom_info.savefile.as_str(),
        saveram_memory.len() / 1024,
        rom_info.has_rtc
    );

    // Prove core 1 and the LED are alive before the game even saves.
    led.write(&LED_WORKER);
    spin(250);
    led.write(&LED_OFF);

    loop {
        let now_ms = Instant::now().as_millis();
        let dirty = save_coordinator::SAVE_DIRTY.swap(false, Ordering::AcqRel);
        let gen = save_coordinator::SAVE_GENERATION.load(Ordering::Acquire);
        let sram_disabled = !save_coordinator::SRAM_ENABLED.load(Ordering::Acquire);
        match engine.poll(dirty, gen, sram_disabled, now_ms) {
            PersistDecision::Persist { generation } => {
                if save_coordinator::SRAM_ENABLED.load(Ordering::Acquire) {
                    spin(POLL_INTERVAL_MS);
                    continue;
                }

                let crc = crc32(saveram_memory);
                engine.begin_persist();
                if engine.should_skip_identical(crc) {
                    engine.on_success(generation, crc);
                    spin(POLL_INTERVAL_MS);
                    continue;
                }
                ...
                let result = (|| {
                    let mut volume = volume_mgr
                        ...
                        .map_err(|_| "save volume")?;
                    let mut root = volume.open_root_dir().map_err(|_| "save root")?;
                    GbSavefile::new(&mut root, rom_info, timesource, &mut *rtc_state_provider)
                        .store(saveram_memory)
                        .map_err(|_| "save write")
                })();
                let still_disabled = !save_coordinator::SRAM_ENABLED.load(Ordering::Acquire);
                match result {
                    Ok(()) if still_disabled => {
                        engine.on_success(generation, crc);
                        led.write(&LED_OK);
                        info!(
                            "save: generation {} persisted to {} ({} bytes SRAM)",
                            generation,
                            rom_info.savefile.as_str(),
                            saveram_memory.len()
                        );
                    ...
                    Ok(()) => {
                        warn!("save: SRAM re-enabled during flush; will retry");
                        engine.on_failure();
                        led.write(&LED_ERR);
                        save_coordinator::SAVE_DIRTY.store(true, Ordering::Release);
                    }
                    Err(e) => {
                        error!("save: persist failed: {}", defmt::Debug2Format(&e));
                        engine.on_failure();
                        led.write(&LED_ERR);
                        save_coordinator::SAVE_DIRTY.store(true, Ordering::Release);
                    }
