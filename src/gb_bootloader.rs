    ...
    pub async fn run(&mut self) -> RomInfo {
        ...
        match volume0 {
            Ok(ref mut volume0) => {
                ...
                root_dir
                    .iterate_dir(|entry| {
                        if used_data + 18 > bootloader_data.len() || num_roms == u8::MAX {
                            return;
                        }
                        if entry.name.extension() == b"GB" || entry.name.extension() == b"GBC" {
...

        {
            ...
            match savefile.load(saveram_memory) {
                Ok(Some(source)) => {
                    info!(
                        "saveram restored from savefile ({})",
                        defmt::Debug2Format(&source)
                    );
                }
                Ok(None) => {
                    info!("no complete savefile; starting with empty SRAM");
                    saveram_memory.fill(0u8);
                }
                Err(error) => {
                    warn!(
